...
const CHANGELOG = [
  ...
  },
  {
    ...
    items: [
      ...
      + '<br>（表記の問題のみであり正しく内部処理は完了している事も確認済みです）',
    ],
  },
  {
    version: 'Beta0.75',
    items: [
      '複数端末で遊びやすくするために同期コードでペアリングをした上でのアップロード及びにダウンロードに対応しました。'
      + '<br>※注意'
      + '<br>セーブデータがある端末にて同期コードを表示し、それをセーブデータを受けたい端末で入力します。'
      + '<br>以後、その認証コードで「最後にアップロードした」セーブデータを共通の認証コード端末にてダウンロードが可能です。'
      + '<br>セーブデータはローカルで保存されており自動ではアップロードされませんのでご注意下さい。',
      'チートに対して、対策を講じました。',
    ],
  },
  {
...
const CASTLE_EFFECT_40_PT_BONUS = 50000;
